...
(`0001-normalize-plugin-skill-join: applied|missing`, `0002-plan-mode-extra-writable: applied|missing`, `0003-session-persist-single: applied|missing`, `0004-disable-builtin-deep-research: applied|missing`, `overlay-pin: wrapper|missing`).
...
| `0004` | `patches/0004-disable-builtin-deep-research.patch` | `[workflows.builtins] deep-research` (compiled default on) + `/settings` and `/plugin` Workflows Space toggle; overlay allowlist | Compiled-in `/deep-research` occupies the slash name so plugin `deep-research:deep-research` cannot advertise bare `/deep-research`. Stock `[workflows] enabled = false` kills every workflow. |
