...
| `0013` | `patches/0013-same-session-compaction-warning.patch` | Persist successful same-session compaction rounds and show a configurable prompt-adjacent warning at round 3; recommend SkillWiki save and new-session handoff from round 4 | Existing `Turn` is a chat/request index and legacy compaction telemetry counts attempts/double-records on auto paths; the warning must be success-accurate and visible without `/context` or a default duplicate footer. |
