...
- `0013-same-session-compaction-warning.patch`: Counts successful same-session compactions separately from legacy attempt telemetry, persists the count, and shows an existing prompt-adjacent warning at the configurable limit (default 3; `0` disables). At the next round it recommends saving SkillWiki progress and handing off to a new session. It does not add a default context footer or create/close sessions.

## Target Commit

- Base commit: `grok-build` commit `4247f661` (`4247f661689354b831191f11eeeac8424993fe3d`) — origin/main 1.0.38; patches 0001–0013 are authored/rebased against this base. Source mode still tracks the moving `origin/main`; this is the patch-authorship/release pin.
