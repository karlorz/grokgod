...
- `0011-ask-question-timeout-action.patch`: `[toolset.ask_user_question] timeout_action = "decline" | "recommended"` (default `decline`) plus env `GROK_ASK_USER_QUESTION_TIMEOUT_ACTION`. Timeout + `recommended` auto-selects `(Recommended)` labels (first match on single-select, all marked on multi-select) with honest auto-select tool text. No `/settings` row. Stacked after 0001–0010 because docs/persist/config_tests already carry earlier patches.

## Target Commit

- Base commit: `grok-build` commit `37949780` (`37949780c144e37df692e3d669051a21fec24f20`) — origin/main 1.0.24; 0003, 0004, 0005, 0007, 0008 rebased onto it 2026-09-10; 0001, 0002, 0006, 0009, 0010 unchanged and verified applying.
...
1. Create a worktree of `grok-build` at base commit `37949780`:
   ```bash
   git -C /path/to/grok-build worktree add /tmp/grokbuild-patch-wt 37949780
   ```
