...
Options:
  ...
  --orca-resume-tag       Tag inner scan with UUID and open Orca resume tab (resumes when scan finishes)
...
if [ "$dry_run" -eq 1 ]; then
  ...
  if [ "$orca_resume_tag" -eq 1 ]; then
    ...
    echo "RESUME TAG: orca terminal create --worktree path:$physical_pwd --title \"grokgod scan resume $(date +%Y-%m-%d)\" --command \"sh $GROKGOD_HOME/resume-$scan_session_id.sh\""
  else
    if [ $# -gt 0 ]; then
...
if [ "$orca_resume_tag" -eq 1 ]; then
  waiter_path="$GROKGOD_HOME/resume-$scan_session_id.sh"
  grok_sessions_dir="${GROK_HOME:-$HOME/.grok}/sessions/$encoded_physical_pwd/$scan_session_id"

  if command -v orca >/dev/null 2>&1; then
    cat << EOF > "$waiter_path"
#!/bin/sh
# Auto-generated by grokgod run --orca-resume-tag. Waits for the inner scan
# session to finish, then resumes it interactively. Safe to delete.
cd '$escaped_physical_pwd' || exit 1
i=0
while [ ! -d '$grok_sessions_dir' ]; do
  i=\$((i + 1))
  if [ "\$i" -gt 600 ]; then
    echo "grokgod waiter: session $scan_session_id never appeared; giving up" >&2
    exit 1
  fi
  sleep 3
done
i=0
while pgrep -f "session-id $scan_session_id" >/dev/null 2>&1; do
  i=\$((i + 1))
  if [ "\$i" -gt 600 ]; then
    break
  fi
...
rm -f "\$0"
exec grok --resume '$scan_session_id'
EOF
    chmod 755 "$waiter_path"

    if orca terminal create \
      --worktree "path:$physical_pwd" \
      --title "grokgod scan resume $(date +%Y-%m-%d)" \
      --command "sh $waiter_path"; then
      echo "note: Orca resume tab created; it will resume session $scan_session_id when the scan finishes" >&2
    else
